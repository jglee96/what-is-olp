...
export function RobotArm({ showWorkspace }: RobotArmProps) {
  ...
  useFrame(() => {
    ...
    if (j1.current) j1.current.rotation.y = a1
    if (j2.current) j2.current.rotation.z = a2
    if (j3.current) j3.current.rotation.z = a3
    if (j4.current) j4.current.rotation.y = a4
    if (j5.current) j5.current.rotation.z = a5
    if (j6.current) j6.current.rotation.y = a6
